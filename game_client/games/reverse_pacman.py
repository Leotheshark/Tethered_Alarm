...
# ─── 後備地圖定義（18 行 × 32 列，TILE_SIZE=60px）──────────────────────────────
# 僅在 maps/<name>.json 缺失或無效時使用；正常情況下地圖由 load_level() 從 JSON 載入。
...
# 預設要載入的地圖（沒設定 MAP_NAME 環境變數時用這個）。要換預設地圖改這一行就好；
# MAP_NAME 環境變數 / debug_game.py --map 仍會臨時覆蓋它。
DEFAULT_MAP_NAME = "sixseven"
...
# ─── 關卡載入：優先讀 maps/<name>.json，否則回退到上面的後備設定 ────────────────
# 編輯器 (tools/map_editor.py) 使用的字元 ↔ 本檔 tile 整數對照。
# P（pellet）已廢棄，載入時一律視為空地(E)。
_CHAR_TO_TILE = {"W": W, ".": E, "P": E, "G": G, "B": B, "S": S, "F": F}
# 反向：int → 字元，供匯出 JSON 用（E 與已廢棄的 P 都輸出為 "."）。
TILE_TO_CHAR = {W: "W", E: ".", P: ".", G: "G", B: "B", S: "S", F: "F"}


def _snap_stations(tile_map, desired):
    """把理想蓄能點吸附到最近空地（後備用；JSON 路徑通常已有明確座標）。"""
...
def _level_from_fallback():
    """用本檔內建後備常數組出完整關卡（pellet→空地、疊迷霧、蓄能站吸附）。"""
...
def _level_from_json(data):
    """把 maps/*.json（編輯器格式）解析成完整關卡。維度需與後備一致。"""
    rows, cols = data["rows"], data["cols"]
    if rows != ROWS or cols != COLS:
        raise ValueError(f"map size {rows}x{cols} != expected {ROWS}x{COLS}")

    tiles = data["tiles"]
    tile_map = [[_CHAR_TO_TILE.get(ch, E) for ch in tiles[r]] for r in range(rows)]

    spawns_raw = data.get("spawns", {})
    spawns = {}
    for color in ("blue", "green", "pink", "red"):
        spawns[color] = tuple(spawns_raw[color]) if color in spawns_raw \
            else _FALLBACK_SPAWN_TILES.get(color, (1, 1))
    pacman_spawn = tuple(spawns_raw.get("pacman", _FALLBACK_PACMAN_SPAWN))

    pairs = {}
    for entry in data.get("button_gate_map", []):
        pairs[tuple(entry["button"])] = tuple(entry["gate"])

    stations_raw = data.get("charge_stations", {})
    if stations_raw:
        stations = {}
        for color in ("blue", "green", "pink", "red"):
            stations[color] = tuple(stations_raw[color]) if color in stations_raw \
                else _FALLBACK_DESIRED_STATIONS.get(color, (1, 1))
    else:
        # 舊地圖沒存蓄能站 → 沿用後備吸附
        stations = _snap_stations(tile_map, _FALLBACK_DESIRED_STATIONS)
    _clear_station_floor(tile_map, stations)
    ...
    return {
        "tile_map": tile_map, "rows": rows, "cols": cols,
        "spawns": spawns, "pacman_spawn": pacman_spawn,
        "button_gate_map": pairs, "charge_stations": stations,
    }


def load_level(name="level_default"):
    """載入關卡：優先 maps/<name>.json，失敗（缺檔/格式錯誤）則回退內建後備。"""
    ...
    try:
        if os.path.isfile(path):
            with open(path, "r", encoding="utf-8") as f:
                data = json.load(f)
            level = _level_from_json(data)
            print(f"[ReversePacman] level loaded from {path}")
            return level
        print(f"[ReversePacman] {path} not found; using built-in fallback map")
    except Exception as e:
        print(f"[ReversePacman] failed to load {path}: {e}; using built-in fallback map")
    return _level_from_fallback()


# 載入一次（可用環境變數 MAP_NAME 指定別張），導出給下方 class 使用的最終地圖資料。
_LEVEL = load_level(os.environ.get("MAP_NAME", DEFAULT_MAP_NAME))
...
def build_tile_map():
    """回傳一份可修改的最終地圖副本（已套用 JSON 或後備設定，含迷霧）。"""
...
class ReversePacman(BaseLogicInterface):
    ...
    def __init__(self, socket_client, player_id_list):
        ...
        # 每位玩家的專屬蓄能點（由載入的關卡提供：JSON 明確擺放，或後備吸附）
    ...
    def get_render_data(self) -> dict:
        ...
        return {
            ...
            "fog_active":  bool(local and local.fog_timer > 0),  # 本地是否處於致盲
    ...
    def get_sync_data(self) -> dict:
        ...
        return {
            ...
            "charge": local.charge,  # 廣播自己的能量條，讓其他人加總判定通關
        }
