...
- **後端 (Backend)**：
  - [ ] 實作基礎 Python-SocketIO 伺服器（`src/server/server.py`），監聽特定 Port (如 5000)。
  - [ ] 定義核心同步協定（Synchronization Protocols）：規範玩家加入、座標、狀態更新的 JSON 封包格式。
  - [ ] 建立「房間管理記憶體結構」，支援 2~4 人輸入相同房號加入，並派發唯一的 `player_id`。
- **前端 (Frontend)**：
  - [ ] 設計大廳等待（LOBBY）、鬧鐘設定與房間輸入畫面的 Pygame UI 基礎線條與佈局（Wireframes）。
  - [ ] 建立 `SocketManager`，封裝斷線、重連函數，確保不阻塞 Pygame 主迴圈。
- **系統層 (System)**：
  - [ ] 引入 `pywin32` 庫，實作基礎 Win32 API 調用，防止電腦進入系統睡眠模式（Sleep Prevention）。
